...
<body class="font-[Poppins] pb-[83px]">
	...
	<section id="user" class="max-w-[1130px] mx-auto flex items-center flex-col gap-[30px] mt-[70px]">
		<div id="title" class="flex items-center gap-[30px]">
			<h1 class="text-4xl leading-[45px] font-bold">user News</h1>
			...
			<div class="flex gap-3 items-center">
				...
				<div class="flex flex-col">
					<p class="text-lg leading-[27px] font-semibold">{{$user->name}}</p>
					<span class="text-[#A3A6AE]">{{$user->occupation}}</span>
				</div>
			</div>
		</div>
				...
				<div
					...
					<div class="thumbnail-container h-[200px] relative rounded-[20px] overflow-hidden">
						<div
							...
							<p class="text-xs leading-[18px] font-bold">{{$news->user->name}}</p>
						</div>
						<img src="{{asset(Storage::url($news->thumbnail))}}" alt="thumbnail photo"
							class="w-full h-full object-cover" />
					</div>
					<div class="flex flex-col gap-[6px]">
						<h3 class="text-lg leading-[27px] font-bold">{{substr($news->name, 0,70)}}{{strlen($news->name)>50 ? '...' : ''}}</h3>
